...
(function () {
	...
	angular.module('SATCI.filters', []).filter("capitalize", function () {
...
(function () {
	...
	angular.module('SATCI.controllers', []).controller('LoginCtrl', function ($auth, $state, $http, $scope, $rootScope, cfpLoadingBar) {
		...
		$scope.saveInstitution = function () {
			...
			Institutions.save($scope.institution).$promise.then(function (data) {
				if (data.success) {
					$scope.full_name = $scope.institution.full_name;
...
(function () {
	...
	angular.module('SATCI.services', []).factory('servingData', function () {
		return {
			data: {}
		};
